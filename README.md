...
```sh
go get -u 'github.com/FarquestSocial/farquest-go@v0.0.1-alpha.0'
```
...
func main() {
	...
	response, err := client.Organizations.New(context.TODO(), farquest.OrganizationNewParams{
...
```go
params := FooParams{
	Name: farquest.F("hello"),

	// Explicitly send `"description": null`
	Description: farquest.Null[string](),

	Point: farquest.F(farquest.Point{
		X: farquest.Int(0),
		Y: farquest.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: farquest.Raw[int64](0.01), // sends a float
	}),
}
```
...
client.Organizations.New(context.TODO(), ...,
	// Override the header
...
`*farquest.Error`. This contains the `StatusCode`, `*http.Request`, and
...
if err != nil {
	...
	panic(err.Error()) // GET "/organizations/create": 400 Bad Request { ... }
}
```
...
client.Organizations.New(
	ctx,
	farquest.OrganizationNewParams{
		AuthRedirectURL: farquest.F("string"),
		CallbackURL:     farquest.F("string"),
		Name:            farquest.F("string"),
	},
...
We also provide a helper `farquest.FileParam(reader io.Reader, filename string, contentType string)`
...
```go
params := FooNewParams{
    ID:   farquest.F("id_xxxx"),
    Data: farquest.F(FooNewParamsData{
        FirstName: farquest.F("John"),
    }),
}
